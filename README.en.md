...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/ArabellaMod/wiki/Home) | [English Home](https://github.com/alkaid616/ArabellaMod/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `ArabellaMod`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `ArabellaMod.json` and update `id`, `name`, `author`, and `description`.
3. Edit `ArabellaModCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `ArabellaMod/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `ArabellaMod.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `ArabellaMod.json` has been synced to the resolved `STS2.RitsuLib` version.
...
| Command | Behavior |
|---|---|
| `dotnet build .\ArabellaMod.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/ArabellaMod` directory.
...
```text
ArabellaMod/
├── ArabellaModCode/   # C# source
├── ArabellaMod/       # Godot resources, localization, and placeholder scenes
├── ArabellaMod.csproj
...
`res://ArabellaMod/...` is the Godot/PCK resource path, mapping to the repository resource directory `ArabellaMod/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
| Property | Value |
|---|---|
| Type | `ArabellaModCharacter` |
| Expected ID | `ARABELLA_MOD_CHARACTER_ARABELLA_MOD_CHARACTER` |
| Starter deck | 4 × `ArabellaModStrike`, 4 × `ArabellaModDefend`, 1 × `ArabellaModRelic` |
...
|---|---|---|
| `ArabellaModStrike` (attack) | character card pool | `ARABELLA_MOD_CARD_ARABELLA_MOD_STRIKE` |
| `ArabellaModDefend` (skill) | character card pool | `ARABELLA_MOD_CARD_ARABELLA_MOD_DEFEND` |
| `ArabellaModRelic` | `ArabellaModRelicPool` | `ARABELLA_MOD_RELIC_ARABELLA_MOD_RELIC` |

### Static placeholder assets

**Images** (`res://ArabellaMod/images/...`):

- `cards/ArabellaModStrike.png`, `cards/ArabellaModDefend.png`: sample card art.
- `relics/ArabellaModRelic.png`: sample relic icon.
- `characters/ArabellaMod_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://ArabellaMod/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `ArabellaMod_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `ArabellaMod_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `ArabellaMod_merchant.tscn` | Merchant | — |
| `ArabellaMod_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `ArabellaMod_character_select_bg.tscn` | Character select background | — |
...
`ArabellaMod.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:
